...
#' @param strategy string to determine the NAs generated by the moving average should be filled with zeros or with the extremities' values. 
#' Possible values: "zero","extremes"
...
ma = function(v,width,align="center",strategy='extremes'){

  # test    ----
  
  # data = linea::sales_ts %>%
  #   get_oecd_data(country_code = 'GB',date_col_name = 'week') %>%
  #   select(week,GB_unemp)
  # v = data$GB_unemp
  # width=11
  # align="center"
  # strategy = 'extremes'
  ...
  if(!strategy %in% c('zero','extremes')){
    message('`ma` Error: strategy input invalid. Valid inputs are:"zero","extremes". Retuning NULL.')
    return(NULL)
  }
  ...
  if(width == 0){
    ...
    if(strategy == 'zero'){
      v = zoo::rollmean(v,width,fill = 0,align = align)
    }else if(strategy == 'extremes'){
      v = v %>% 
        zoo::rollmean(k = width, fill = NA, align = align) %>%
